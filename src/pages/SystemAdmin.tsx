...
import { ArrowLeft, Database, ShoppingBag, Users, Sliders, LogOut, Settings, Calendar, Bot } from 'lucide-react';
...
import { AICodeExporter } from '../components/Admin/AICodeExporter'; // [新增] 引入 AI 匯出元件
import { supabase } from '../supabase';

export const SystemAdmin: React.FC = () => {
    const navigate = useNavigate();
    // [修改] 增加 'ai' 選項
    const [activeTab, setActiveTab] = useState<'users' | 'products' | 'divination' | 'config' | 'booking' | 'ai'>('users');
            ...
            <div className="flex flex-1 overflow-hidden">
                <aside className="w-64 bg-white border-r border-gray-200 flex flex-col shrink-0 overflow-y-auto">
                    <nav className="p-4 space-y-2 flex-1">
                        ...
                        <div className="border-t border-gray-100 my-4"></div>
                        <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-2">開發者工具</div>
                        
                        {/* [新增] AI 協作匯出按鈕 */}
                        <button onClick={() => setActiveTab('ai')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-bold transition-all ${activeTab === 'ai' ? 'bg-emerald-50 text-emerald-600 ring-1 ring-emerald-200' : 'text-gray-600 hover:bg-gray-50'}`}><Bot size={18} /> AI 協作匯出</button>
                    </nav>
                    <div className="p-4 border-t border-gray-200 text-xs text-center text-gray-400 shrink-0">System v2.4</div>
                </aside>
                ...
                <main className="flex-1 overflow-hidden relative bg-slate-50 flex flex-col">
                    ...
                    {/* [新增] AI 協作匯出面板 */}
                    {activeTab === 'ai' && <div className="w-full h-full overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300"><AICodeExporter /></div>}
                </main>
            </div>
        </div>
    );
