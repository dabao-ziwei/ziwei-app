...
import { Plus, Edit2, Trash2, Save, X, ShoppingBag, BarChart3, Calendar, FileSpreadsheet, Eye } from 'lucide-react';
    ...
    const loadTransactions = async () => {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                ...
                base_points_snapshot, bonus_points_snapshot,
                user:profiles(email)
            `)
    ...
    const handleExport = () => {
        ...
        const csvData = transactions.map(t => {
            ...
            return {
                ...
                '取得點數': (t.base_points_snapshot + t.bonus_points_snapshot),
                '狀態': t.status,
                '訂單 ID': t.id
            };
        });
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-0">
                    {loading ? <div className="flex justify-center py-20"><span className="animate-spin mr-2">⏳</span> 讀取中...</div> : (
                        <table className="w-full text-sm text-left whitespace-nowrap">
                            <thead className="bg-gray-100 text-gray-600 font-bold sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="p-3 text-right">取得點數</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {transactions.map(t => {
                                    const dateObj = new Date(t.created_at);
                                    const totalPoints = (t.base_points_snapshot || 0) + (t.bonus_points_snapshot || 0);
                                    return (
                                        <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="p-3 text-right font-mono text-purple-600 font-bold">{totalPoints}</td>
...
export const ProductManagement: React.FC = () => {
    ...
    const [loading, setLoading] = useState(false);
    const [editingPack, setEditingPack] = useState<Partial<PointPack> | null>(null);
        ...
        try {
            ...
            transactions.forEach((t: any) => {
                ...
                if (t.status === 'SUCCESS') {
                    ...
                    statsMap[packId].revenue += t.price_ntd_snapshot || 0;
    ...
    const handleSave = async () => {
        ...
        const payload = {
            ...
            price_ntd: editingPack.price_ntd,
            base_points: editingPack.base_points,
            bonus_points: editingPack.bonus_points,
        ...
        if (editingPack.id) {
            await supabase.from('point_packs').update(payload).eq('id', editingPack.id);
        } else {
            await supabase.from('point_packs').insert(payload);
        }
        setEditingPack(null);
        loadPacks();
    };
    ...
    const handleDelete = async (id: string) => {
        if (confirm('確定刪除此商品？')) {
            await supabase.from('point_packs').delete().eq('id', id);
            loadPacks();
        }
    };
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                {statsLoading ? <div className="text-center py-4 text-gray-400">載入統計中...</div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="bg-gradient-to-br from-purple-600 to-indigo-600 rounded-lg p-4 text-white shadow-lg">
                            ...
                            <div className="text-3xl font-black font-mono">
                                ${salesStats.reduce((acc, s) => acc + s.revenue, 0).toLocaleString()}
                            </div>
                        </div>
                        <div className="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-lg p-4 text-white shadow-lg">
                            ...
                            <div className="text-3xl font-black font-mono">
                                {salesStats.reduce((acc, s) => acc + s.count, 0)} <span className="text-sm font-normal">筆</span>
                            </div>
                        </div>
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {salesStats.map((stat, idx) => (
                                            <tr key={idx} className="hover:bg-white transition-colors">
                                                ...
                                                <td className="p-2 text-right font-mono text-green-600 font-bold">${stat.revenue.toLocaleString()}</td>
            ...
            <div>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <ShoppingBag className="text-blue-600" /> 商品上架管理
                    </h2>
                    <button 
                        onClick={() => setEditingPack({ is_active: true, base_points: 100, bonus_points: 0, price_ntd: 100 })} 
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {packs.map(pack => (
                        <div key={pack.id} className={`bg-white rounded-xl p-5 border shadow-sm relative transition-all hover:shadow-md ${pack.is_active ? 'border-gray-200' : 'border-gray-200 opacity-60 bg-gray-50'}`}>
                            ...
                            <div className="text-3xl font-black text-purple-600 font-mono mb-1">
                                {pack.base_points + pack.bonus_points} <span className="text-sm text-gray-500 font-sans font-normal">點</span>
                            </div>
                            ...
                            {pack.label && (
                                <div className="absolute top-4 right-12 bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded-full">
                                    {pack.label}
                                </div>
                            )}
                            ...
                            <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                                ...
                                <button 
                                    ...
                                    className="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg hover:bg-purple-50 hover:text-purple-600 font-bold text-sm flex items-center justify-center gap-2 transition-colors"
                                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-bold text-gray-800">{editingPack.id ? '編輯商品' : '新增商品'}</h3>
                            <button onClick={() => setEditingPack(null)}><X className="text-gray-400 hover:text-gray-600"/></button>
                        </div>
                        
                        <div className="space-y-4">
                            <div><label className="block text-sm font-bold text-gray-700 mb-1">商品名稱</label><input type="text" className="w-full p-2 border rounded-lg" value={editingPack.name || ''} onChange={e => setEditingPack({...editingPack, name: e.target.value})} /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-sm font-bold text-gray-700 mb-1">價格 (NTD)</label><input type="number" className="w-full p-2 border rounded-lg" value={editingPack.price_ntd || 0} onChange={e => setEditingPack({...editingPack, price_ntd: parseInt(e.target.value)})} /></div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-sm font-bold text-gray-700 mb-1">基本點數</label><input type="number" className="w-full p-2 border rounded-lg" value={editingPack.base_points || 0} onChange={e => setEditingPack({...editingPack, base_points: parseInt(e.target.value)})} /></div>
                                <div><label className="block text-sm font-bold text-gray-700 mb-1">贈送點數</label><input type="number" className="w-full p-2 border rounded-lg" value={editingPack.bonus_points || 0} onChange={e => setEditingPack({...editingPack, bonus_points: parseInt(e.target.value)})} /></div>
                            </div>
                        ...
                        <div className="mt-6 flex gap-3">
                            <button onClick={() => setEditingPack(null)} className="flex-1 py-2 border rounded-lg text-gray-600 hover:bg-gray-50">取消</button>
                            <button onClick={handleSave} className="flex-1 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold flex items-center justify-center gap-2"><Save size={18}/> 儲存</button>
                        </div>
                    </div>
                </div>
            )}
