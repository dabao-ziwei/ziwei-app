...
export const UserManagementModal: React.FC<Props> = ({ isOpen = true, onClose }) => {
  ...
  const checkIsSuperAdmin = (email?: string | null) => (email || '').trim().toLowerCase() === SUPER_ADMIN_EMAIL;
  ...
  const processedProfiles = useMemo(() => {
    ...
    return result.sort((a, b) => {
        const aVal = a[sortConfig.key as keyof UserProfile];
        const bVal = b[sortConfig.key as keyof UserProfile];
  ...
  const executeBulkCharts = async () => {
      ...
      setIsBulkUpdating(true);
      if (typeof adminBulkUpdateMaxCharts !== 'function') {
         alert('錯誤：找不到 adminBulkUpdateMaxCharts 函式，請檢查 db.ts');
         setIsBulkUpdating(false);
         return;
      }
  ...
  const containerClass = isModalMode 
      ? "fixed inset-0 z-[100] bg-slate-50 flex flex-col overflow-hidden text-gray-800 animate-in slide-in-from-bottom-2 duration-200" 
      : "w-full h-full bg-slate-50 flex flex-col overflow-hidden relative text-gray-800";

  return (
    <div className={containerClass}>
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 bg-white items-center shrink-0">
            {isModalMode && (
                <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full text-gray-500 mr-2">
                    <ChevronLeft size={24} />
                </button>
            )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm text-gray-500 text-sm">
                        <tr>
                            ...
                            <SortableHeader label="VIP 到期日" sortKey="accessExpiry" className="text-center w-28" />
                        ...
                        {paginatedData.map(user => {
                            ...
                            return (
                                <tr key={user.id} className={`hover:bg-gray-50 group transition-colors ${selectedIds.has(user.id) ? 'bg-blue-50/50' : ''}`}>
                                    ...
                                    <td className="py-3 px-2 text-center text-sm font-mono text-purple-600 font-bold">
                                        {user.accessExpiry ? new Date(user.accessExpiry).toLocaleDateString() : '-'}
                                    </td>
        ...
        {selectedIds.size > 0 && (
            <div className="bg-blue-600 text-white px-6 py-3 border-t border-blue-700 flex flex-row items-center justify-between shrink-0 shadow-lg z-30">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={() => setShowBulkExpiryModal(true)} className="px-4 py-2 bg-white text-blue-600 rounded-lg font-bold hover:bg-gray-100 flex items-center gap-2 shadow-md transition-colors whitespace-nowrap text-sm">
                        <CalendarClock size={16}/> 批次展延期限
                    </button>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden">
                    ...
                    <div className="flex border-b border-gray-200 bg-white shrink-0">
                        ...
                        <button onClick={() => setActiveTab('history')} className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'history' ? 'border-purple-600 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><History size={16}/> 歷史交易紀錄</button>
                    </div>
                                    ...
                                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                        <table className="w-full text-xs text-left">
                                            <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-200">
                                                <tr><th className="p-3">時間</th><th className="p-3">方案</th><th className="p-3">金額</th><th className="p-3">狀態</th></tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-100">
                                                ...
                                                {userTransactions.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-gray-400">無購買紀錄</td></tr>}
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <div>
                                    <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2"><History size={16}/> 系統調整紀錄 (Ledger)</h4>
                                    {historyLoading ? <Loader2 className="animate-spin text-gray-400"/> : (
                                        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                            <table className="w-full text-xs text-left">
                                                <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-200">
                                                    <tr><th className="p-3">時間</th><th className="p-3">類型</th><th className="p-3 text-right">變動</th><th className="p-3">原因/備註</th></tr>
                                                </thead>
                                                <tbody className="divide-y divide-gray-100">
                                                    {userLedger.map(row => (
                                                        <tr key={row.id} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="p-3 text-gray-600 truncate max-w-[200px]" title={row.reason}>{row.reason}</td>
                                                        </tr>
                                                    ))}
                                                    {userLedger.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-gray-400">無紀錄</td></tr>}
                                                </tbody>
                                            </table>
                                        </div>
                                    )}
