...
import { X, Save, Edit2, Search, ChevronLeft, ChevronRight, Loader2, Shield, Trash2 } from 'lucide-react';
import { getAllProfilesWithStats, updateProfile, toggleUserBan, deleteUserProfile, type UserProfile } from '../db';
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
                <tr className="text-gray-500 text-sm">
                  ...
                  <th className="py-3 px-2 text-center w-24">狀態</th>
                  <th className="py-3 px-2">角色</th>
                  <th className="py-3 px-2 text-center">現有/上限</th>
                ...
                {paginatedData.map(user => {
                  ...
                  return (
                    <tr key={user.id} className={`hover:bg-gray-50 group ${user.isBanned ? 'bg-red-50/30' : ''}`}>
                      <td className="py-3 px-4 text-sm font-bold text-gray-700 truncate max-w-[220px]" title={user.email}>
                        {user.email}
                      </td>
                      ...
                      <td className="py-3 px-2 text-center">
                        {isEditing ? (
                           <div className="flex items-center justify-center gap-1">
                               <span className="text-blue-600 font-bold">{user.activeCount}</span>
