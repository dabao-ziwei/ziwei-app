...
import { Network, ArrowLeft, Eye, RefreshCw, Users, Repeat, Clock } from 'lucide-react';
import { DateInput } from './DateInput'; // 確保您有此組件，若無可換回純文字顯示
...
interface CenterInfoBoardProps {
  ...
  historyStack: Client[]; // 新增：用於判斷是否顯示返回按鈕

  onHistoryBack: () => void; // 新增
...
export const CenterInfoBoard: React.FC<CenterInfoBoardProps> = ({
    ...
    return (
        // 這裡保持 col-span-2 row-span-2，由 ChartBoard 的 Grid 系統控制位置
        <div className="col-span-2 row-span-2 flex border border-gray-300 bg-white z-10 relative overflow-hidden">
            
            {/* 左側：個人資料 (動態寬度) */}
            <div className={`${hasRelations ? 'w-full md:w-[35%]' : 'w-full'} h-full flex flex-col p-3 border-r border-gray-100 bg-white z-20 shadow-sm relative transition-all duration-300`}>
                
                {/* 如果沒有右側關聯圖，返回按鈕顯示在這裡 */}
                {!hasRelations && historyStack.length > 0 && (
                    <button 
                        onClick={onHistoryBack}
                        className="absolute top-2 left-2 z-50 flex items-center gap-1 px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-bold rounded transition-colors"
                    >
                ...
                <div className="flex-1 flex flex-col items-center justify-start text-center gap-1 mt-2">
                    {/* 姓名 */}
                    ...
                    {/* 格局 + 命主/身主 (改為單行顯示) */}
                    {chartData && (
                        <div className="text-xs text-gray-500 font-medium mt-2 space-y-1">
                    ...
                    {/* 緊湊生辰 */}
                    {chartData && (
                        <div className="grid grid-cols-3 gap-x-2 gap-y-0 text-[11px] text-gray-500 mt-3 border-t border-gray-100 pt-2 w-full max-w-[180px]">
                ...
                <div className="mt-auto pt-2 flex gap-1">
                    <div className="flex bg-slate-100/80 rounded-lg p-1 gap-1">
                        ...
                        {!isDaXian && !isLiuNian && (
                            <>
                                <button 
                                    ...
                                    className={`px-2 py-1 text-[10px] font-bold rounded transition-colors flex items-center gap-1 ${showTwin ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:bg-white'}`}
                                >
                                    <Users size={12} /> 雙胞胎
                                </button>
                        ...
                        <button 
                            ...
                            className={`px-2 py-1 text-[10px] font-bold rounded transition-colors flex items-center gap-1 ${showInverted ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:bg-white'}`}
                        >
                            <Repeat size={12} /> 顛倒盤
                        </button>
                        ...
                        {isLiuNian && (
                            <>
                                ...
                                <button 
                                    ...
                                    className={`px-2 py-1 text-[10px] font-bold rounded transition-colors flex items-center gap-1 ${showSmallLimit ? 'bg-green-600 text-white shadow-sm' : 'text-gray-500 hover:bg-white'}`}
                                >
                                    <Clock size={12} /> 小限
                                </button>
                            </>
            ...
            {hasRelations && (
                <div className="hidden md:block w-[65%] h-full relative bg-slate-50 overflow-hidden">
                    ...
                    {historyStack.length > 0 && (
                        <button 
                            onClick={onHistoryBack}
                            className="absolute top-2 left-2 z-50 flex items-center gap-1 px-3 py-1.5 bg-white/90 hover:bg-white text-gray-700 text-xs font-bold rounded-lg border border-gray-300 shadow-sm transition-all backdrop-blur-sm"
                        >
                            <ArrowLeft size={12} />
