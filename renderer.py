...
def render_full_chart_html(calc, data, d_idx, l_off, focus_idx):
    ...
    for idx, r, c in layout:
        ...
        if focus_idx != -1:
            ...
            elif idx in [(focus_idx+4)%12, (focus_idx+8)%12]: cell_style_extra = "background-color: rgba(255, 247, 230, 0.9);"
            elif idx == (focus_idx+6)%12: cell_style_extra = "background-color: rgba(246, 255, 237, 0.9);"
        ...
        # 修正：改回 javascript:void(0); 這是唯一被證明能送出訊號（導致迴圈）的方法
        # 配合 app.py 的防迴圈閘門，這就是正確解答
        cells_html += f"""
