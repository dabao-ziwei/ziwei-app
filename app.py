...
st.set_page_config(page_title="專業紫微斗數排盤", page_icon="🔮", layout="wide")
apply_style()
...
with st.sidebar:
    ...
    for p in st.session_state.db:
        db_options[p['id']] = f"{p['name']} ({p['gender']})"
    
    # 使用 selectbox 確保點擊絕對有效
    selected_id = st.selectbox(
        "選擇或搜尋命主", 
        options=list(db_options.keys()), 
        format_func=lambda x: db_options[x],
        index=0 if st.session_state.current_id == 0 else list(db_options.keys()).index(st.session_state.current_id)
    )
...
if st.session_state.current_id != 0:
    ...
    if data:
        ...
        # 操作區：三方四正 (極簡化)
        col_focus, _ = st.columns([2, 5])
        with col_focus:
            opts = [(-1, "隱藏連線")] + [((calc.ming_pos - i)%12, PALACE_NAMES[i]) for i in range(12)]
            f_idx = st.selectbox("三方四正", options=[x[0] for x in opts], format_func=lambda x: next(n for i,n in opts if i==x), label_visibility="collapsed")
        ...
        # 這裡我們用 Streamlit 的 columns 加上 CSS hack 來模擬一條連續的時間軸
        # 為了效能與簡潔，直接顯示按鈕
