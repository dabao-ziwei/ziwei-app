...
if st.session_state.page == 'list':
    ...
    with c1: search_kw = st.text_input("🔍 搜尋", placeholder="姓名/分類...")
    with c2:
        if st.button("➕ 新增", use_container_width=True):
            st.session_state.current_id = 0; st.session_state.page = 'chart'; st.rerun()
                ...
                for item in items:
                    ...
                    with col_btn:
                        if st.button("開啟", key=f"open_{item['id']}", use_container_width=True):
                            st.session_state.current_id = item['id']; st.session_state.page = 'chart'
                            st.session_state.sel_daxian_idx = -1; st.session_state.sel_liunian_offset = -1; st.session_state.focus_palace_idx = -1 
                            st.rerun()
...
# === 頁面 2: 命盤 ===
    ...
    if st.session_state.current_id != 0:
        ...
        v_name, v_gen, v_cat = (rec['name'], rec['gender'], rec['category']) if rec else ("", "女", "")
    ...
    if btn_save or btn_calc:
        ...
        if not i_name or y==0: st.error("資料不完整")
            ...
            if btn_save:
                ...
                save_db(st.session_state.db); st.session_state.current_id = pkt['id']; st.session_state.temp_preview_data = None; st.rerun()
            if btn_calc: st.session_state.temp_preview_data = pkt
    ...
    if data:
        ...
        st.markdown(f'<div class="chart-container">{svg_html}<div class="zwds-grid">{cells_html}{get_center_html(data, calc_obj)}</div></div>', unsafe_allow_html=True)
