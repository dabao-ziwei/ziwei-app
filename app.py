...
# 1. 頁面設定 (必須在所有指令之前)
st.set_page_config(page_title="專業紫微斗數", page_icon="🔮", layout="wide")
apply_style()
...
with st.sidebar:
    ...
    # [選擇命主] - 使用 Selectbox 確保穩定切換
    # 建立選項清單：ID: 顯示名稱
    opts = {0: "➕ 新增命盤"}
    for p in st.session_state.db:
        opts[p['id']] = f"{p['name']} ({p['gender']})"
    ...
    # [編輯/新增區塊]
    st.markdown("---")
    edit_label = "新增資料" if st.session_state.current_id == 0 else "編輯資料"
    with st.expander(edit_label, expanded=(st.session_state.current_id == 0)):
        # 取得當前資料
...
if data:
    ...
    # 2. 三方四正 (上方工具列)
    c_tools = st.container()
    with c_tools:
        # 簡單的一行工具列，不佔空間
        col_focus, _ = st.columns([2, 6])
        with col_focus:
    ...
    # 5. 時間軸 (流年列表 - 僅在大限選中時出現)
    if not is_pure:
        st.markdown('<div class="timeline-container" style="border-top: 1px solid #ccc;">', unsafe_allow_html=True)
