...
if 'page' not in st.session_state: st.session_state.page = 'list' # 預設進入列表頁
...
if st.session_state.page == 'list':
    ...
    with c1:
        search_kw = st.text_input("🔍 搜尋客戶 (姓名/分類)", placeholder="輸入關鍵字...")
    with c2:
        if st.button("➕ 新增命盤", use_container_width=True):
            st.session_state.current_id = 0 # 0 代表新增
    ...
    if not st.session_state.db:
        st.info("目前沒有資料，請點擊右上方「新增命盤」。")
    
    for cat, items in categories.items():
        if items:
            with st.expander(f"📁 {cat} ({len(items)})", expanded=True):
                for item in items:
                    # 使用 columns 模擬 list item，並放一個 button 進入
...
# ==========================================
    ...
    if btn_save or btn_calc:
        ...
        if not i_name or y==0: st.error("資料不完整")
            ...
            if btn_save:
                ...
                st.session_state.current_id = pkt['id'] # 確保 ID 同步
    ...
    if data:
        ...
        # === 7. 三方四正操作區 ===
        # 由於無法直接點擊格子，我們用一個選擇器，預設選命宮
        # 為了方便，我們顯示相對宮位名稱 (命宮, 財帛...) 而不是地支
        
        # 建立選項列表： (index, 宮名)
        focus_opts = [(-1, "無 (隱藏連線)")]
        # 從命宮開始順數的宮位列表
        for i in range(12):
            # i=0是命宮位置(benming_pos)
            # 我們要讓選單顯示「命宮」「兄弟」...
            # 實際對應的 index 是 (benming_pos - i) % 12 (逆時針)
